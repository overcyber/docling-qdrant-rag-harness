...
def release_ingest_reservation(tenant_id: str, ingest_fingerprint: str) -> None:
    try:
        redis.Redis.from_url(settings.redis_url).delete(
            ingest_reservation_key(tenant_id, ingest_fingerprint)
        )
    except Exception:
        # Reservation cleanup must never turn an indexed document into a failed job.
        pass
    ...
    *,
    ...
    try:
        ...
        with path.open("rb") as fh, httpx.Client(
            timeout=httpx.Timeout(float(settings.parser_timeout_seconds), connect=15.0)
        ) as http:
            resp = http.post(
        ...
        self.update_state(
            state="PROGRESS",
            meta={"stage": "embedding", "progress": 35, "chunks": len(chunks)},
        )
        ...
        with httpx.Client(
            timeout=httpx.Timeout(float(settings.embedder_timeout_seconds), connect=15.0)
        ) as http:
        ...
        for i, (chunk, dense, sparse) in enumerate(zip(chunks, dense_all, sparse_all)):
            ...
            payload = {
                ...
                "embedding_models": embedding_identity or {
