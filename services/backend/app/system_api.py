...
def ready() -> dict[str, Any]:
    ...
    for name, url in {
        "parser": f"{settings.parser_url}/health",
        "embedder": f"{settings.embedder_url}/health",
    }.items():
        try:
...
@router.get(
    ...
    summary="Get effective non-secret runtime configuration",
)
...
@router.get(
    "/v1/config/chunkers",
    dependencies=[Depends(require_auth)],
    summary="Describe the three supported Docling chunkers and their request schemas",
)
    ...
    return {
        ...
        "types": {
            ...
            "hierarchical": {
                ...
                "fields": {
                    "merge_list_items": "boolean; retained for upstream compatibility",
                    "always_emit_headings": "boolean",
                },
            ...
            "line_based": {
                ...
                "fields": {
                    "tokenizer_model": "string",
                    "max_tokens": "integer 32..32768",
                    "prefix": "string repeated in chunks",
                    "omit_prefix_on_overflow": "boolean",
                },
                "example": {
                    "type": "line_based",
                    "max_tokens": 120,
        ...
        "upload_wrapper_example": {
            "chunking": {"type": "hybrid", "max_tokens": 120},
            "pdf": {"do_ocr": True, "do_table_structure": True, "do_cell_matching": True},
            "conversion": {"max_num_pages": 1000},
            "deduplicate": True,
        },
    }
...
@router.post(
    ...
    dependencies=[Depends(require_auth)],
    summary="Validate a processing_options object",
    description=(
        "Validates the same processing_options schema accepted by multipart document ingestion. "
        "This endpoint exists partly so the complete discriminated chunking union is visible in OpenAPI/Swagger."
    ),
)
def validate_processing_options(options: ProcessingOptions) -> dict[str, Any]:
    return {
        ...
        "defaults": settings.public_dict()["chunking_defaults"],
    }
